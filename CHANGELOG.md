...
- **Major USB compatibility correction** (source: Cedric via Rocco Corsi, CoCo Discord #esp32-coco channel): the real constraint isn't "USB 1.1" generally, it's **USB Low Speed (1.5 Mbps) specifically** — the device's USB Soft Host library can't handle Full Speed (12 Mbps, most USB 1.1 gear) or High Speed (480 Mbps, USB 2.0+). This explains the hit-or-miss compatibility even among old USB 1.1 keyboards/joysticks. Also documented a possible workaround: a USB isolator adapter with an **ADuM4160** chip and a 1.5/12 Mbps speed-lock switch can force a Full Speed device down to Low Speed (won't help High Speed devices; cost-vs-benefit worth weighing against just trying another keyboard). Updated: keyboards.md (full explanation + workaround), joysticks-and-controllers.md, hardware-specs.md, features.md, keyboard-shortcuts.md — all now say "USB Low Speed" instead of the less precise "USB1."
- **community-links.md**: added the CoCo Discord's **#esp32-coco channel** under Primary source, as an active discussion venue with Cedric (no direct channel link available — Discord requires server membership to link channels). Updated the general Discord server entry to note the channel's existence.
- **keyboards.md / hardware-specs.md**: resolved the connector-type TODO — the USB ports are classic **USB-A**.
- **keyboards.md**: removed the "Original CoCo/Tandy keyboards" section — the original CoCo keyboard is fully enclosed in the CoCo unit and connects via ribbon cable, not a realistic option to wire into ESP32-COCO, so not worth documenting as a possibility.
